...
/** A class containing drive commands */
    ...
    Command returnCommand =
        Commands.run(
                () -> {
                  ...
                  Rotation2d rotationVelocity =
                      getAngularSpeedFromJoysticks(
                          xAngleSupplier.getAsDouble(),
                          yAngleSupplier.getAsDouble(),
                          drive.getRotation().getDegrees());

                  // Convert the commanded speeds into the correct units for the drivetrain
                  double xSpeedDelivered =
                      throttle * linearVelocity.getX() * DriveConstants.maxSpeedMetersPerSec;
                  double ySpeedDelivered =
                      throttle * linearVelocity.getY() * DriveConstants.maxSpeedMetersPerSec;
                  double rotDelivered =
                      rotationVelocity.getRadians() * DriveConstants.maxAngularSpeed;

                  // Convert to field relative speeds & send command
                  ChassisSpeeds speeds =
                      ChassisSpeeds.fromFieldRelativeSpeeds(
                          xSpeedDelivered, ySpeedDelivered, rotDelivered, drive.getRotation());
