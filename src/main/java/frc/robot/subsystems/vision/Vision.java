...
public class Vision extends SubsystemBase {
  ...
  public void periodic() {
    ...
    for (int cameraIndex = 0; cameraIndex < io.length; cameraIndex++) {
      ...
      for (var observation : inputs[cameraIndex].poseObservations) {
        ...
        boolean rejectPose =
            observation.tagCount() == 0 // Must have at least one tag
                ...
                || Math.abs(pose.getZ()) > VisionConstants.maxZError // Must have realistic Z
                // coordinate

                // Must be within the field boundaries
                || pose.getX() < 0.0
                || pose.getX() > VisionConstants.aprilTagLayout.getFieldLength()
                || pose.getY() < 0.0
                || pose.getY() > VisionConstants.aprilTagLayout.getFieldWidth();
      ...
      String camPath = "Vision/Camera" + cameraIndex;

      // Log camera datadata
      Logger.recordOutput(camPath + "/TagPoses", tagPoses.toArray(new Pose3d[0]));
      Logger.recordOutput(camPath + "/RobotPoses", robotPoses.toArray(new Pose3d[0]));
      Logger.recordOutput(
          camPath + "/RobotPosesAccepted", robotPosesAccepted.toArray(new Pose3d[0]));
      Logger.recordOutput(
          camPath + "/RobotPosesRejected", robotPosesRejected.toArray(new Pose3d[0]));
